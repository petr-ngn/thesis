...
Regarding the latter approach, the feature selection is based on a specific machine learning model and follows a greedy search approach by evaluating all the possible combinations of features against the evaluation metric criteria \citep{Verma2020}.
...
Instead, the feature selection is performed with Sequential Feature Selection (henceforth SFS). Such method is iteratively adding (or removes) features the set sequentially. 
...
\For{$model \in \textit{models}$}
    ...
    \State $\textit{best\_features} \gets \textsc{SequentialFeatureSelection}(\textit{optimized\_model})$
\EndFor
\end{algorithmic}
\end{algorithm}
