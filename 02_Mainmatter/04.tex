...
Particularly, for each model a rank score is calculated in order to determine the final rank. The lower rank score indicates better model performance. For particular model $M$, the rank score can be calculated as weighted average of the individual ranks as follows:
...
By doing so, the sample size used for training is increased. As such, the re-training (or so called recalibration) model leads (or is expected to lead) to markedly improved model performance \citep{de2023predicting}.
