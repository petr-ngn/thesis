...
Such metric is also known as True Positive Rate (TPR) or Sensitivity and it describes in relative terms how many dactual \textit{True} instances the model has correctly predicted out of all the \textit{True} instances. Thus, the goal is to minimize the number of False Negatives, i.e., number of defaulters which the model has classified as non--defaulters.
\textbf{TBD}
...
F1 score incorporates both Recall and Precision into a single value. Is it defined as a harmonic mean of these two metrics and the goal is to minimize False Positives and False Negatives at the same time.
\textbf{TBD}
...
\begin{equation}\label{eq}
    \text{Kolmogorov Smirnov} = \max_{x \in \mathbb{R}} \left| \hat{F}_n \left(x \right) - F \left(x \right) \right|
\end{equation}
...
\begin{equation}\label{eq}
    \text{Somers' D} = \frac{\tau \left(X, Y\right)}{\sqrt{\tau \left(X, X\right) \tau \left(Y, Y\right)}}
\end{equation}
