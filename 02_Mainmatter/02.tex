...
Howeover, default can have various forms and definitation. 
...
Basel Commitee on Banking Supervision (BCBS) introduced Capital Accord Basel II and Basel III in 2004 and 2010, respectively \citep{shakdwipee2017basel}, with a goal to set higher risk management and internal control standards for banks \citep{witzany2017credit}.
...
While the aim of IFRS 9 regards the financial reporting which should follow a general purpose to provide information to those outside the firm to support decision usefulness, whereas Basel II/III seeks to decrease the frequency and cost of bank failuers and to protect the financial system as whole by limiting the frequency and cost of systemic crises \citep{beerbaum2015credit}.
...
\begin{itemize}\setlength\itemsep{0em}
	...
	\item \textbf{Supervised Learning} - Process reflecting the abilty of an algorithm to generelize knowledge from the data having with target or label cases, so that the algorithm can be used to predict new (unlabelled) cases \citep{berry2020supervised}.
...
Moreover, another advanced machine learning techniques have been presented, particularly, ADASYN oversampling for dealing with imblanaced, i.e., skewed distribution of the target variable (\autoref{sec:adasyntheory}), Optimal Binning for data preprocessing using discretization with respect to the target variable (\autoref{sec:optbinningtheory}),
